...
const AP_Param::GroupInfo NavEKF::var_info[] = {
    ...
    // @Description: This parameter controls which height sensor is used by the EKF during optical flow navigation (when EKF_GPS_TYPE = 3). A value of 0 will cause it to always use baro altitude. A value of 1 will cause it to use range finder if available.
